...
def generate_summaries_or_translations(
    ...
    **generate_kwargs,
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(model_state_dict_path.replace('pytorch_model.bin', '')).to(device) #from_pretrained(model_state_dict_path.replace('pytorch_model.bin', '')+'config.json').to(device) #
    model.eval()
    if fp16:
        model = model.half()
    #if model_state_dict_path is not None:
    #    model.load_state_dict(torch.load(model_state_dict_path, map_location=torch.device('cpu')))
    tokenizer = AutoTokenizer.from_pretrained(model_state_dict_path.replace('pytorch_model.bin', '')) #model_name
...
def run_generate(verbose=True):
    ...
    parser.add_argument(
        "--n_obs", type=int, default=1000000, required=False, help="How many observations. Defaults to all."
    )
    ...
    print('Number of examples to be evaluated:', len(examples))
    ...
    scores['SciBertScore'] = calculate_bert_scores(output_lns, reference_lns, verb=True)
