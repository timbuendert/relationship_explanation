...
class DataTrainingArguments:
    ...
    context_style: str = field(
        metadata={
            "help": "The context style of input dataset."}
    )
    data_dir: str = field(
        metadata={
...
def main():
    ...
    print(f'Length of tokenizer before: {len(tokenizer)}')

    ADDED_SPECIAL_TOKENS = ["<DOC_SEP>"]
    if data_args.context_style == 'intro_tfidf':
        ADDED_SPECIAL_TOKENS.append('<TFIDF>')
    if data_args.context_style == 'intro_entity':
        ADDED_SPECIAL_TOKENS.append('<TFIDF>')
        ADDED_SPECIAL_TOKENS.append('<ENT>')
    #ADDED_SPECIAL_TOKENS = ['<TFIDF>', '<DOC_SEP>']
    
    #['@Single_summ', '@Narrative_cite', '@Multi_summ', '@Reflection', '@Transition'] 

    num_added_tokens = tokenizer.add_special_tokens(
        {'additional_special_tokens': ADDED_SPECIAL_TOKENS})
    print(f'Length of tokenizer after: {len(tokenizer)}')
    print(tokenizer)
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(model_args.model_name_or_path if os.path.isdir(model_args.model_name_or_path) else None) #resume_from_checkpoint
        print('done')
